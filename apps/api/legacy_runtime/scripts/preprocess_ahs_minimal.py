"""Minimal AHS preprocessing: keep candidate grounding columns and write compact parquet output."""
...
def write_minimal_parquet(
    ...
    if not source_csv.exists():
        raise FileNotFoundError(
            ...
            "Run extraction first: python scripts/extract_and_inspect_ahs.py"
        )
...
def main() -> None:
    """Run minimal AHS preprocessing."""
    project_root = Path(__file__).resolve().parents[1]

    ahs_csv = project_root / "data" / "raw" / "ahs" / "extracted" / "ahs2023n.csv"
    lookups_dir = project_root / "data" / "processed" / "lookups"
    metadata_dir = project_root / "data" / "processed" / "metadata"

    write_minimal_parquet(
        source_csv=ahs_csv,
        output_parquet=lookups_dir / "ahs_minimal.parquet",
        candidate_columns=AHS_CANDIDATE_COLUMNS,
        dataset_label="AHS national",
        summary_json_path=metadata_dir / "ahs_minimal_summary.json",
    )
