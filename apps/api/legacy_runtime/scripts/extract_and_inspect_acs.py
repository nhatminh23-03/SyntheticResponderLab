"""Extract ACS PUMS archives and write a small schema-inspection metadata summary."""
...
def unzip_archive(archive_path: Path, destination_dir: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "Invalid or corrupted ACS archive detected. "
            f"Archive: {archive_path}. "
            "Please re-download using: bash scripts/download_acs_pums.sh"
        ) from error
...
def main() -> None:
    """Extract ACS archives and write metadata summary JSON."""
    project_root = Path(__file__).resolve().parents[1]
    raw_dir = project_root / "data" / "raw" / "acs_pums"
    extracted_dir = raw_dir / "extracted"
    output_dir = project_root / "data" / "processed" / "metadata"
    output_path = output_dir / "acs_pums_summary.json"
    ...
    for archive_path in archive_paths:
        if not archive_path.exists():
            raise FileNotFoundError(
                "Required ACS archive missing. "
                f"Expected file: {archive_path}\n"
                "Run: bash scripts/download_acs_pums.sh"
            )
    ...
    summary = {
        "dataset": "ACS PUMS California 2024 5-year",
        "source_archive_paths": [str(path) for path in archive_paths],
    ...
    print(f"ACS extraction + inspection complete. Summary written to: {output_path}")
