  ...
  const transcriptAnswers = useMemo(
    () =>
      Object.entries(selectedTranscript?.answers ?? {}).filter(
        ([qid, answer]) => qid !== "additional_thoughts" && Boolean(answer)
      ),
    [selectedTranscript]
  );
      ...
      <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
          ...
          {isLoading && (
            <RevealOnScroll delay={0.04}>
              <GlassPanel className="p-6">
                <p className="text-sm text-app-muted">Loading interview insights and transcript context...</p>
              </GlassPanel>
            </RevealOnScroll>
          )}
          ...
          {latestRun?.status === "completed" && pairs.length > 0 && (
            <RevealOnScroll delay={0.08}>
              <GlassPanel className="p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs uppercase tracking-[0.14em] text-app-muted">Continue The Interview</p>
                    <p className="mt-2 max-w-3xl text-sm leading-6 text-app-muted">
                      Pick a persona and continue the interview in character. Changing persona or transcript source
                      resets the current conversation so each thread stays grounded in one respondent.
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedPair && (
                      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
                        <p className="text-[0.68rem] uppercase tracking-[0.12em] text-app-muted">Persona Snapshot</p>
                  ...
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4">
                      ...
                      <div className="mt-3 max-h-48 space-y-3 overflow-y-auto pr-1">
                        {transcriptAnswers.slice(0, 4).map(([qid, answer]) => (
                          <div key={qid}>
                        ...
                        {!transcriptAnswers.length && (
                          <p className="text-xs text-app-muted">No saved transcript answers for this persona yet.</p>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="max-h-[24rem] space-y-3 overflow-y-auto pr-1">
                        {chatMessages.length === 0 ? (
                          <p className="text-sm leading-6 text-app-muted">
                            Ask a follow-up like &ldquo;What would make you trust the install process more?&rdquo;
                            or &ldquo;How would your spouse react to this purchase?&rdquo;
                          </p>
                        ) : (
                    ...
                    <form className="space-y-3" onSubmit={handleChatSubmit}>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <p className="text-xs text-app-muted">
                          Replies use the saved transcript plus persona context from the latest interview run.
                        </p>
                        <button
                          type="submit"
        ...
        <div className="space-y-4 lg:sticky lg:top-[calc(var(--nav-height)+1.5rem)]">
          ...
          <RevealOnScroll delay={0.1}>
            <GlassPanel className="p-5">
              <p className="mb-3 text-xs uppercase tracking-[0.14em] text-app-muted">About This Section</p>
              <p className="text-xs leading-5 text-app-muted">
                Theme extraction summarizes the Model A corpus. Persona follow-up chat continues one selected
                respondent in character using the saved transcript and persona profile from Interview Synthesis.
              </p>
            </GlassPanel>
          </RevealOnScroll>

          {latestRun?.persona_count != null && (
            <RevealOnScroll delay={0.12}>
              <div className="flex justify-center gap-6 px-1 py-3">
                <div className="text-center">
                  <p className="text-2xl font-bold text-app-text">{latestRun.persona_count}</p>
