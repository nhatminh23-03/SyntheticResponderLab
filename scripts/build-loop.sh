...
P4.4|analysis/R/arm_c.R ALREADY EXISTS and was committed in 744cefb — read it first and FINISH it rather than rewriting it. Three things are missing and the task is not done without them. (1) analysis/tests/test_arm_c.R, in the same shape as test_arm_a.R and test_arm_b.R, asserting at minimum: the aytm: prefix join resolves all 42 shared questions (a join on Question_Mapping.csv's abbreviated student_column resolves only 23 and must not be used); the student sample is 256 rows; the AYTM anchor-stripping turns "5 - Extremely appealing" into "5" so the 1-5 scales compare; the harmonised income and age brackets have equal levels on both sides; and the run is reproducible under ARM_C_SEED. Every assertion must be able to fail — do not write a test that passes on an empty or NULL result. (2) Wire Arm C into analysis/run_all.R so the single reproducible HTML report covers it alongside Arm A and Arm B. (3) Confirm in the test that the two required caveats are present in the emitted report text, not merely in code comments: the samples are screened differently (student PQ1 contains No, AYTM screened those out before fielding), and AYTM is a national panel while the student survey is Southern-California-based with no geography field to control for. Do not touch the read-only CSVs at ~/dev/aytm-real-data/neo_smart_living/student_sample/ and never put them in a prompt.
